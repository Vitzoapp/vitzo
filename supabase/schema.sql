...
BEGIN
    ...
    IF v_actual_pin = p_entered_pin THEN
        UPDATE orders SET delivery_status = 'delivered' WHERE id = p_order_id;
        RETURN TRUE;
    ELSE
        RETURN FALSE;
    END IF;
...
BEGIN
  ...
  FOR item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    SELECT stock INTO v_current_stock FROM products WHERE id = (item->>'product_id')::UUID FOR UPDATE;
    IF v_current_stock < (item->>'quantity')::INTEGER THEN RAISE EXCEPTION 'Not enough stock'; END IF;
    UPDATE products SET stock = stock - (item->>'quantity')::INTEGER WHERE id = (item->>'product_id')::UUID;
